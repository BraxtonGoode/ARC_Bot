    ...
    try {
      ...
      try {
        ...
        return interaction.reply(
          createErrorReply(`Couldn't find tip "${tipFileName}".`)
        );
      }
      ...
      if (providedBy && providedBy.trim() !== '') {
        components.push({
          type: ComponentType.TextDisplay,
          content: `Provided by ${providedBy} ${
            unixTimestamp ? `- Last updated on <t:${unixTimestamp}:D>` : ''
          }`,
        });
      ...
      if (image && image.trim() !== '') {
        ...
        try {
          ...
          const gallery = new MediaGalleryBuilder().addItems((item) =>
            item
              .setDescription(name || tipFileName)
              .setURL(`attachment://${image}`)
          );

          const insertIndex = components.findIndex(
            (c) =>
              c.content &&
              (c.content.startsWith('Provided by') ||
                c.content.startsWith('Last updated on'))
          );
          if (insertIndex >= 0) {
            components.splice(insertIndex, 0, gallery, {
              type: ComponentType.Separator,
            });
          } else {
          ...
          console.warn(
            `Couldn't find image "${image}" for tip "${tipFileName}", skipping..`
          );
        }
      }
